...
export default function ChatAssistant({ initialQuery = '' }: { initialQuery?: string }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-140px)] min-h-[400px]">
      {/* Header */}
      <div className="flex items-center justify-between px-4 md:px-5 py-3 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)]/60 backdrop-blur-xl rounded-t-xl">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white flex-shrink-0" style={{ background: 'linear-gradient(135deg, var(--accent-amber), #d97706)', boxShadow: '0 0 16px var(--accent-amber-glow)' }}>
            AI
          </div>
          <div>
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">AI Assistant</h3>
            <p className="text-[10px] text-[var(--text-muted)] hidden sm:block">Groundwater analysis powered by JAL-DRISHTI</p>
          </div>
        </div>
        <button
          onClick={clearChat}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 md:px-5 py-4 space-y-4 bg-[var(--bg-deep)]">
        {showSuggestions && (
          <div className="flex flex-col items-center justify-center h-full text-center py-8 md:py-12 animate-fade-in">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'linear-gradient(135deg, var(--accent-amber-glow), var(--accent-teal-glow))', border: '1px solid var(--border-medium)' }}>
              <svg className="w-6 h-6 md:w-7 md:h-7 text-[var(--accent-amber)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <p className="text-sm text-[var(--text-muted)] max-w-md mb-2 px-4">
              Ask questions about groundwater assessment data across Indian states and districts.
            </p>
            <p className="text-[11px] text-[var(--text-faint)] max-w-sm mb-6 px-4" style={{ fontFamily: 'var(--font-mono)' }}>
            ...
            <button
              ...
              className="mb-6 flex items-center gap-2.5 px-5 md:px-6 py-2.5 md:py-3 rounded-xl text-sm font-semibold text-[var(--bg-deep)] disabled:opacity-50 transition-all duration-300 group"
              style={{
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg w-full px-2">
              {SUGGESTED_QUESTIONS.map((q, i) => (
                <button
                  ...
                  className="text-left px-3 md:px-4 py-2.5 md:py-3 rounded-xl text-sm text-[var(--text-secondary)] border border-[var(--border-subtle)] hover:border-[var(--accent-amber)]/30 hover:bg-[var(--accent-amber)]/5 transition-all duration-200 group animate-fade-in-up"
      ...
      {/* Input */}
      <div className="px-4 md:px-5 py-3 md:py-4 border-t border-[var(--border-subtle)] bg-[var(--bg-surface)]/60 backdrop-blur-xl rounded-b-xl">
        <div className="flex items-center gap-3 bg-[var(--bg-elevated)]/50 border border-[var(--border-subtle)] rounded-xl px-3 md:px-4 py-2 focus-within:border-[var(--accent-amber)]/30 focus-within:ring-1 focus-within:ring-[var(--accent-amber)]/20 transition-all">
          ...
          <input
            ...
            disabled={isTyping}
            className="flex-1 bg-transparent text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none disabled:opacity-50 min-w-0"
          />
          <button
...
function MessageBubble({ message, onFollowup }: { message: ChatMessage; onFollowup: (text: string) => void }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
      <div className={`max-w-[90%] md:max-w-[85%] ${isUser ? 'order-2' : ''}`}>
        {/* Message */}
        <div
          className={`px-3 md:px-4 py-2.5 md:py-3 rounded-2xl text-sm leading-relaxed ${
            isUser
